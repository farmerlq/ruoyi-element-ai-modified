...
import { ref, watch, withDefaults, defineProps, defineEmits, computed } from 'vue';
...
function handleClick() {
  ...
  else if (props.trigger === 'clickTarget') {
    // clickTarget模式：点击触发元素时显示，点击其他地方时隐藏（通过onClickOutside处理）
    if (!showPopover.value) {
      show();
    }
  }
...
// 添加默认的z-index以确保popover内容显示在其他元素之上
.popover-content-box {
  z-index: 1000;
}
</style>