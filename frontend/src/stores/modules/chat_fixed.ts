...
export const useChatStore = defineStore('chat', () => {
  ...
  const setChatMap = (id: string, data: ChatMessageVo[]) => {
    ...
    chatMap.value[id] = data.map((item: ChatMessageVo, index: number) => {
      ...
      // 从数据库字段获取思考类事件和其他事件
      const reasoningEvents = (item as any).reasoning_events || [];
      const otherEvents = (item as any).other_events || [];
      ...
      if (Array.isArray(reasoningEvents) && reasoningEvents.length > 0) {
        reasoningEvents.forEach((event: any) => {
          if (event.thought) {
            ...
            thoughtContent += event.observation + '\n';
          ...
          if (event.tool && event.tool_input) {
            thoughtContent += `\n\n[工具调用: ${event.tool}]\n`;
            thoughtContent += `参数: ${JSON.stringify(event.tool_input, null, 2)}\n`;
          }
        });
        // 移除末尾的换行符
      ...
      if (Array.isArray(workflowEvents)) {
        ...
        workflowEvents.forEach((event: any) => {
          ...
          // 也支持从data字段中提取toolInfo
          else if (event.data && event.data.event === 'agent_thought') {
            const data = event.data;
            if (data.tool || data.tool_name || data.name || (data as any).toll) {
              const toolInfo = {
                name: data.tool || data.tool_name || data.name || (data as any).toll || '',
            ...
            else if (data.toolInfo) {
              const isDuplicate = toolInfoList.some((toolInfoItem: any) => 
                toolInfoItem.name === data.toolInfo.name && 
                JSON.stringify(toolInfoItem.input) === JSON.stringify(data.toolInfo.input)
              );
              
              if (!isDuplicate && data.toolInfo.name) {
                toolInfoList.push(data.toolInfo);
              }
            }
          }
        });
      ...
      // 确保历史消息中的工具调用信息也正确传递
      if (!isUser && processedItem.toolInfoList && Array.isArray(processedItem.toolInfoList)) {
        // 为历史消息中的工具调用添加一个版本号，确保组件能正确渲染
        (processedItem as any).toolInfoVersion = processedItem.toolInfoList.length;
        
        // 确保至少有一个有效的工具调用
        if (processedItem.toolInfoList.length > 0) {
          console.log('Tool info list for message:', processedItem.toolInfoList);
        }
      }

      // 确保reasoning_content正确传递
      if (!isUser && (processedItem.reasoning_content || processedItem.thought_content)) {
        // 保持展开状态以显示思考过程
        processedItem.thinkCollapse = false;
      }
