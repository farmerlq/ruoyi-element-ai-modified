...
# 按ID和position对数据进行分组
def group_tool_calls(data):
    grouped = {}
    
    for item in data:
        # 只处理有工具名称的事件
        if item["tool"]:
            key = f"{item['id']}-{item['position']}"
            ...
            if item["tool_input"] and item["tool_input"] != '{}':
                grouped[key]["tool_input"] = item["tool_input"]
            
            if item["observation"] and item["observation"] != '{}':
...
for i, tool_call in enumerate(parsed_tool_calls):
    ...
    print(f"  输入: {'存在' if tool_call['input'] else '不存在'}")
    print(f"  响应: {'存在' if tool_call['observation'] else '不存在'}")
    
    if tool_call['observation']:
        try:
            parsed_observation = json.loads(tool_call['observation'])
            print(f"  响应内容预览: {str(parsed_observation)[:100]}...")
        except json.JSONDecodeError:
            print(f"  响应内容预览: {tool_call['observation'][:100]}...")